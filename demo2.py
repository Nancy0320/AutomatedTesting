...
from settings import MODEL, BRAND, TYPE
...
class AddVehicleNumberTest(minium.MiniTest):
    ...
    #     print("TYPE:", TYPE)
    #
    #     vehicle_model: str = random.choice(MODEL)
    #     vehicle_brand: str = random.choice(BRAND)
    #     vehicle_type: str = random.choice(TYPE)
    #     # 选择款式
    #     self._click_element("view", "电动自行车选项", inner_text={vehicle_model})
    #
    #     # 选择品牌
    #     self._click_element("view", "凤凰品牌", inner_text={vehicle_brand})
    #
    #     # 选择型号
    #     self._click_element("view", "型号1", inner_text={vehicle_type})
    #     # 随机选择车辆信息（类型检查器会推断出正确的类型）
    #
    #     print(f"随机选择: 车型={vehicle_model}, 品牌={vehicle_brand}, 型号={vehicle_type}")
    #
    #     # 确认添加
    ...
    def _select_vehicle_info(self):
        ...
        # 选择款式（关键修复：移除集合语法，直接传递字符串）
        self._click_element("view", f"{vehicle_model}选项", inner_text=vehicle_model)

        # 选择品牌
        self._click_element("view", f"{vehicle_brand}品牌", inner_text=vehicle_brand)

        # 选择型号
        self._click_element("view", f"型号{vehicle_type.split('型号')[-1]}", inner_text=vehicle_type)
