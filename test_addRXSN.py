...
class AddVehicleNumberTest(minium.MiniTest):
    ...
    def test_add(self, test_num):
        ...
        try:
            ...
            # self.app.switch_tab("page/index/index")
            self._click_element("image", "返回按钮", timeout=5)
    ...
    def _bind_device(self):
        ...
        # 等待绑定按钮容器
        self.page.wait_for(".scanTheCode_view", max_timeout=10)

        # 查找所有.bind_device元素，然后通过文本过滤
        bind_devices = self.page.get_elements(".scanTheCode_view")
        target_device = None
